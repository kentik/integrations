	...
	c.Registry.Each(func(baseName string, i interface{}) {
		...
		if tagsSer == "" {
			...
			if len(pts) > MIN_FOR_HOST {
				...
				tags = append(tags, "cid="+strings.Join(pts[pLen-4:pLen-3], "_"))
				tags = append(tags, "did="+strings.Join(pts[pLen-4:pLen-1], "_"))
				tags = append(tags, "sid="+strings.Join(pts[pLen-1:], "_"))
			} else {
				tags = append(tags, "host="+shortHostnameBase)

				if len(pts) >= 4 {
					tags = append(tags, "cid="+strings.Join(pts[1:2], "_"))
					tags = append(tags, "did="+strings.Join(pts[3:4], "_"))
				}
			}
			if c.Extra != nil {
